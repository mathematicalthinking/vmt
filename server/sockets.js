  ...
  io.sockets.on('connection', (socket) => {
    ...
    socket.on('SETTINGS_CHANGE', async (roomId, settings, callback) => {
      ...
      try {
        ...
        const message = new Message({
          ...
          room: roomId,
          text: 'Room settings were changed',
          autogenerated: true,
          messageType: 'TEXT',
          timestamp: Date.now(),
        });
    ...
    const killZombies = async (roomId) => {
      ...
      await releasedControl.forEach(async (tab) => {
        ...
        message.save();
        socket.to(roomId).emit('RELEASED_CONTROL', message);
        socket.emit('RELEASED_CONTROL', message); // so that person who does action gets message
