...
const generateTakeControlMessage = (context, event) => {
...
const takeControlSocketService = (context, event) => (callback) => {
  const message = generateTakeControlMessage(context, event);
  socket.emit('TAKE_CONTROL', message, (msg, response) => {
    if (response === 'Success') {
      callback({ type: 'SUCCESS', message });
      if (typeof event.callback === 'function') event.callback(message);
    } else {
      callback({ type: 'REJECTED' });
      console.log(`TAKE_CONTROL REJECTED: ${msg}`);
    }
  });
};
  ...
  return [
    {
      ...
      states: {
        [controlStates.NONE]: {
          ...
          on: {
            [controlEvents.CLICK]: 'invokeService',
          },
          states: {
            invokeService: {
              invoke: {
                src: takeControlSocketService,
              },
              on: {
                SUCCESS: {
                  target: `#${STRATEGY.DEFAULT}.${controlStates.ME}`,
                  actions: assign({
                    lastMessage: (_, event) => event.message,
                  }),
                },
                REJECTED: {
                  target: `#${STRATEGY.DEFAULT}.${controlStates.NONE}`,
                  // Optionally add an action here to handle the rejection
                },
              },
            },
          },
