...
class Replayer extends Component {
  ...
  messageLog = this.updatedLog.filter(entry => entry.text)
  componentDidMount() {
  ...
  goToTime = (percent) => {
    ...
    this.constructionLog.some((entry, i ) => {
      if (entry.relTime > timeElapsed) {
        // we've found the most recent construction
        constructionLogIndex = i === 0 ? 0 : i - 1;
        return true;
      } return false;
    })
    let messageLogIndex;
    this.messageLog.some((entry, i) => {
      console.log(entry.relTime)
      if (entry.relTime > timeElapsed) {
        console.log('entry: ', entry.relTime)
        // We've found the most recent message
        messageLogIndex = i === 0 ? 0 : i - 1;
        return true;
      }  else if (i === this.messageLog.length - 1) messageLogIndex = i;
      return false
    })
    this.setState({timeElapsed, logIndex, constructionLogIndex, messageLogIndex, playing: false, changingIndex: true,})
  ...
  render() {
    ...
    return (
      <WorkspaceLayout
        ...
        chat = {() => <ChatReplayer event={event} log={this.messageLog} index={this.state.messageLogIndex} skipping={this.state.changingIndex}
          reset={this.reset}/>}
        // chat={() => <div>chat</div>}
        replayer={() =>
          (<ReplayControls
