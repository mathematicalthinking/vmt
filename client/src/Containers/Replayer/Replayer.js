...
class Replayer extends Component {
  ...
  playing = () => {
    console.log("LOG INDEX: ", this.state.logIndex)
    let eventDuration;
    let currentEvent;
    let nextEvent;
    let updatedIndex;
    // IF this is the first event
    if (this.state.logIndex === -1) {
      eventDuration = 0;
      currentEvent = this.log[0];
      updatedIndex = 1;
    } else {
      currentEvent = this.log[this.state.logIndex];
      console.log("logINdex: ", this.state.logIndex)
      nextEvent = this.log[this.state.logIndex + 1];
      // console.log(nextEvent)
      eventDuration = nextEvent.timestamp - currentEvent.timestamp;
      if (eventDuration > MAX_WAIT) {
        eventDuration = BREAK_DURATION
        console.log('event duration longer than max wait, new duration: ', BREAK_DURATION)
      }
    ...
    setTimeout(() => {
      ...
      this.setState(prevState => ({
        logIndex: updatedIndex || prevState.logIndex + 1,
