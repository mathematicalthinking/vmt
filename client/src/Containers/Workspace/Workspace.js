...
class Workspace extends Component {
  ...
  handleScreenChange = (screenNum) => {
    ...
    const { controlState, populatedRoom } = this.props;
    const { currentTabId } = controlState;
    const { tabs } = this.state;
    const currentTab = tabs.find((tab) => tab._id === currentTabId);
    ...
    // add screen change to log
    const tabNumber = tabs.findIndex((tab) => tab._id === currentTabId) + 1;
    this.addToLog(
      {
        _id: mongoIdGenerator(),
        user: { _id: 'VMTBot', username: 'VMTBot' },
        // if there's only 1 tab, don't include the tab number
        text:
          tabs.length === 1
            ? `Screen changed to ${screenNum + 1}`
            : `Screen changed to ${screenNum + 1} on tab #${tabNumber}`,
        autogenerated: true,
