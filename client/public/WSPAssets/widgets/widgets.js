...
// Private functions here, utility functions first
  ...
  function deepEquals(a, b) {
    ...
    if (a && b && typeof a === 'object' && typeof b === 'object') {
      ...
      for (var key in a) {
        if (a.hasOwnProperty (key) && b.hasOwnProperty (key) && !deepEquals(a[key], b[key]))
          return false;
      }
    } 
    return true;
  ...
  function findWidgetButton (canvas) {  // Returns the button iff there's a single widget_button in the container
  ...
  function targetControllerToDoc (newNode) {
    ...
    if (buttonNode) {  // If the Widgets button exists, show it only if some widgets are enabled
    ...
    if (nodeChanged) { // Reposition the widget only when the node changes
      ...
      if (targetNode) { // remove handlers from the old node
      ...
      $targetNode.on ("WillPlayTool.WSP", function () { //  attach handlers to the new node
    ...
    if (sketchChanged) {  // even if the node's not changed, a page switch detaches the widgets
      if ($widgetParent.parent().length) {
        console.log ("targetControllerToDoc found the widgets still attached after the sketch was changed!");
      }
      $sketch.prepend ($widgetParent);
      }
      // setupDropdownHandlers ();  THIS IS NEEDED ONLY DURING WIDGET INTIALIZATION
    if (nodeChanged) {  // Reposition the widgets for a new node, but not for a new sketch in the same node
  ...
  function injectButtonContent (canvas) { // Find any uninitialized .widget_button element matching this doc id, and initialize the buttons.
    ...
    if (button) { // There's a single widget button inside the canvas' container; set it to target the widget to this sketch.
...
/* ***** LABEL WIDGET ***** */
  ...
  function generateNewLabel (gobj) {  // handles measures and (transformed images using namedFromLabel)
    var newLabel;
    savePool ();
  ...
  function changeText (newText, newOrigin) {  // Respond to a user's change of the gobj's text or nameOrigin. Return the resulting text.
    ...
    if (gobj.hasLabel) {    // geometric objects live on the canvas layers
      ...
      if (newTextEmpty) // Cannot set label to empty string
        newText = gobj.label;
      gobj.setLabel(newText, {showLabel: (newTextEmpty ? false : true), wasUserIntiated: true});
  ...
  labelWidget.handleTap = function (event, context) { // cache label info to enable restoring it if user cancels
    ...
    function initForNewGobj () {      //  Handle the first tap on the current gobj or label
      ...
      if (toGobj.style.nameOrigin === "namedFromLabel" && (toGobj.label.match(/namedFromLabel/) || !toGobj.label)) {
  ...
  function toggleLabel (show) { // Toggle the visibility of this label and set checkbox
    ...
    if (gobj.hasLabel) {
      ...
      if (show && !prevState) {
        ...
        if (labelWidget.inputElt[0].value === "") // if input is empty when we show a label, set the input to the label
 ...
 */
  ...
  deleteWidget.handleTap = function (event, context) {
    ...
    if (gobj) {
      deleteWidget.progenyList = gobj.sQuery.sketch.gobjList.compileDescendants (gobj);
  ...
  function makeWidgetHTML () {
    ...
    var text = "<div id=\"widget\" class=\"clearfix\">\
...
<br><a id=\"downloadLink\" href=\"\" download=\"\">Download</a><br>";
  ...
  function makeWidget (data) {  // from some source we have the html for the widget
    var $data = $("<div>");
    ...
    $widget.find("img").attr('src', function (index, attr) { // Set the img src attributes to their correct url's
...
var PAGENUM = (function() { // define the PAGENUM namespace, for the code that creates and handles page number controls
  ...
  function setToolEnabling (sketchDoc) {  // Only need to call this once to set css classes, UNLESS
    ...
    tools.forEach (function (tool) {
      ...
      classes = classes.replace(/\bpage_toggle\b/, " ").replace(/\bp_\d+\b/g, " ").trim();
      ...
      $node.toggle(enabledPages[0] === "all" || enabledPages.includes(+sketchDoc.focusPage.metadata.id));
    });
  }
  ...
  function injectButtonElements (sketchDoc) { // Find any uninitialized .page_buttons element with html matching this doc id, and initialize the buttons.
    var sketchNode = sketchDoc.canvasNode[0],
        ...
        newContent, btnAreaContent, $utilMenu;
    if ($control.length === 1 && pageCtlEnabled) {
      ...
      if (sketchDoc.docSpec.pages.length > 1) {
        ...
        if (!$btnArea.length) { // An active page control must be inside the button_area
...
var UTILMENU = (function() {  // define the UTILMENU namespace.
  ...
  function sketchDocFromContainerEl ($el) {  // return the sketch doc in the common parent of $el and the sketch
    var found, $canvas;
  ...
  function createUtilMenu () {    // Create the menu itself and returns the file-content div
    var scriptPath =  WIDGETS.getScriptPath(),
        newContent = '<div class="util-menu-btn util-menu">',
        $button = $(this).parent().find(".util-menu-btn");

    function distItem (units, name) { // cm, inches, or pixels
      return  '<div class="util-menu-item util-menu util-length" data-unit="' + name + '">' + "\u00A0 " + units + '</div>';
    }

    function angleItem (units, name) { // degrees(=/-), degrees(+), or radians
      return  '<div class="util-menu-item util-menu util-angle" data-unit="' + name + '">' + "\u00A0 " + units + '</div>';
    }
    ...
    function adjustUnitsObjAndPrecision (gobj, unitsObj, newUnit, deltaP) {
      ...
      gobj.style.precision = clampPrecision (gobj.style.precision + unitsObj.power * deltaP);
    ...
    function setLengthPref (event) {
      var doc = sketchDocFromContainerEl (event.target),
          ...
          deltaP = 0,
          newPrecision, gobjs, gobj;
      if (newUnit && newUnit !== oldUnit) { // Update the unit in the sketch and the spec, to save it on export
        if (newUnit === 'pix') {
          deltaP = -2;
        } else if (oldUnit === 'pix') {
          deltaP = 2;
        ...
        doc.pageData[sketch.metadata.id].spec.preferences.units.length = newUnit;
        if (deltaP) { // update the sketch-wide default precision
          ...
          doc.pageData[sketch.metadata.id].spec.preferences.precision.length = newPrecision;
        }
        gobjs = doc.sQuery ('[genus="DistanceMeasure"],[genus="DistanceParameter"],[genus="AreaMeasure"],[genus="Function"]');
        for (var i = 0; i < gobjs.length; i++) {
          gobj =  gobjs[i];
          if (gobj.unitsObject.length) {
            adjustUnitsObjAndPrecision (gobj, gobj.unitsObject.length, newUnit, deltaP);
            if (GSP.isParameter(gobj) && gobj.unitsObject.length) {    // update the multiplier used to display correct values
              gobj.unitMultiplier = GSP.units.convertToBaseFromUnitObject(1, gobj.unitsObject); 
    ...
    function setAnglePref (event) {
      ...
      if (newUnit && newUnit !== oldUnit) { // Update the unit in the sketch and the spec, to save it on export
        ...
        if (deltaP) {
          ...
          doc.pageData[sketch.metadata.id].spec.preferences.precision.angle = newPrecision;
        }
        gobjs = doc.sQuery ('[genus="AngleMeasure"],[genus="AngleParameter"],[genus="Function"]');
        for (var i = 0; i < gobjs.length; i++) {
          gobj =  gobjs[i];
          ...
          if (gobj.unitsObject.angle) { // angle measure
            adjustUnitsObjAndPrecision (gobj, gobj.unitsObject.angle, newUnit, deltaP);
            if (GSP.isParameter(gobj) && gobj.unitsObject.angle) {    // update the multiplier used to display correct values
              gobj.unitMultiplier = GSP.units.convertToBaseFromUnitObject(1, gobj.unitsObject);
              gobj.isExpressionDirty = true;
              gobj.fnExpression = undefined;
              gobj.parsedInfix = undefined;
    ...
    newContent += distItem('cm', 'cm') + distItem('inches', 'in.') + distItem ('pixels', "pix");
    newContent += '<hr>';
    newContent += angleItem('degrees(+/-)', 'signed deg') + angleItem('degrees(+)', 'deg') + angleItem ('radians', 'rad');
    ...
    $button.replaceWith (newContent);
    $(this).parent().find(".util-length").click (function () {setLengthPref(event);});
    $(this).parent().find(".util-angle").click (function () {setAnglePref(event);});
    $(this).parent().find(".util-menu-content").mouseleave (hideUtilMenu);
  }
  
  function enableFileCommands (event, context) {  // hide/show the upload/download commands on LoadDocumnent.WSP
    // Create the menu if it doesn't exist.
    var doc = context.document,
        $button = $(doc.canvasNode).parent().find(".util-menu-btn"),
        $fileContent, downEnabled, upEnabled, eitherEnabled;
    if (!$button.length) {
      return;  // no button, so no util menu for this sketch
    }
  ...
  function updateUnitPrefs ($button) {   // check preferred units on DidChangeCurrentPage.WSP. $button can be the utility button or a descendant.
    ...
    prefs = doc.focusPage.preferences.units;
      ...
      function angleUnitToItem (unit) {
        switch (unit) {
          case "signed deg" : return 'degrees(+/-)';
    ...
    function hideUtilMenu () {  // No need to find the open util menu; they can all be hidden. (Problem for multiple users?)
      $('.util-menu-content').hide();
    }

    function downloadSketch (sketchDoc, title) {
      var pageData = sketchDoc.pageData,
          scriptOutput, varName, json, dataBlob, objUrl;
      ...
      link.download = title;
      link.click();
      sketchDoc.event( 'DownloadDocument',
        { document: sketchDoc},
        { fileName: title}
      );
    ...
    function prepareDownload () {   // set up handlers for the download dialog box
      var defaultName = $(curSketchNode).data('fileName') + $(curSketchNode).data('fileExt');
