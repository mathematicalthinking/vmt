...
describe('create each type of resource', function(){
  ...
  it('creates a room from an activity', function(){
    cy.getTestElement('tab').contains('Rooms').click()
    cy.getTestElement('create-room').click()
    cy.get('input[name=name]').type('{selectall} {backspace}').type('room from activity')
    cy.get('input[name=description]').type('{selectall} {backspace}').type(room.description)
    cy.get('button').contains('copy existing activities').click()
  ...
  it('creates an activity from an existing activity', function(){
    ...
    cy.get('input[name=name]').type('{selectall} {backspace}').type('activity from existing activity')
    cy.get('input[name=description]').type('{selectall} {backspace}').type(room.description)
    cy.get('button').contains('copy existing activities').click()
    cy.getTestElement('ACTIVITY 2-checkbox').click()
    cy.get('button').contains('next').click()
